...
// 				$delivery_type = get_user_meta($user->ID, 'simbatfa_delivery_type', true);
	...
	public function authUserFromLogin($params) {
		...
		if (!$this->isCallerActive($params)) return true;
		...
		if (!$this->isActivatedForUser($user_ID)) return true;
			
		if (!$this->isActivatedByUser($user_ID)) {

			if (!$this->isRequiredForUser($user_ID)) return true;
...
// 		$tfa_last_login = get_user_meta($tfa_creds_user_id, 'tfa_last_login', true); // Unused
		...
		if (in_array($this->hash($user_code, $tfa_creds_user_id), $tfa_last_pws)) return false;
	...
	private function encrypt($key, $string, $iv) {
		...
		if (function_exists('openssl_encrypt')) {
			...
			return $this->is_php_71 ? @mcrypt_encrypt(MCRYPT_RIJNDAEL_128, $key, $string, MCRYPT_MODE_CBC, $iv) : mcrypt_encrypt(MCRYPT_RIJNDAEL_128, $key, $string, MCRYPT_MODE_CBC, $iv);
		}
		throw new Exception('One of the mcrypt or openssl PHP modules needs to be installed');
	}
